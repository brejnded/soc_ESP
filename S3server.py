...
def generate_leaderboard_html(leaderboard):
    ...
    <head>
        ...
        <style>
            ...
            .button-link { display: inline-block; padding: 10px 20px; background-color: #4CAF50; color: white; text-decoration: none; border-radius: 5px; }
            .button-link2 { display: inline-block; padding: 10px 20px; background-color: #4CAF50; color: white; text-decoration: none; border-radius: 5px; }
            .reset-button { padding: 10px 20px; background-color: #f44336; color: white; border: none; border-radius: 5px; cursor: pointer; text-decoration: none; display: inline-block;}
            table { border-collapse: collapse; width: 450px; margin: 60px auto 20px auto; }
    ...
    <body>
        <div class="header-buttons">
            ...
            <button onclick="resetLeaderboard()" class="reset-button">Reset</button>
        </div>
...
def generate_admin_html(config):
    ...
    <head>
        ...
        <style>
            body {{ font-family: sans-serif; }} /* Use double {{ and }} to escape */
            .container {{ width: 400px; margin: 20px auto; }}
            label {{ display: block; margin-top: 10px; }}
            input[type="number"], select {{ width: calc(100% - 16px); padding: 8px; margin-top: 5px; margin-bottom: 10px; box-sizing: border-box; }}
            .button-container {{ text-align: center; margin-top: 20px; }}
            .button-container2 {{ text-align: center; margin-top: 20px; margin-left: 20px; margin-right: 20px; }}
            button, .button-link {{ display: inline-block; padding: 10px 20px; background-color: #4CAF50; color: white; text-decoration: none; border-radius: 5px; border: none; cursor: pointer; font-size: 1em; }}
            .button-link {{ background-color: #008CBA; margin-top: 10px; display: block; width: calc(100% - 40px); margin-left: auto; margin-right: auto; text-align: center;}}
            .button-link2 {{ background-color: #4CAF50; margin-top: 10px; display: block; width: calc(100% - 40px); margin-left: auto; margin-right: auto; text-align: center;}}
        </style>
    </head>
    <body>
        <div class="container">
            ...
            <form action="/save_answers" method="post">
                ...
                <input type="number" id="penalty" name="penalty" value="{}" required>""".format(
        penalty
    )
    for i in range(1, 16):
        question_num = str(i)
